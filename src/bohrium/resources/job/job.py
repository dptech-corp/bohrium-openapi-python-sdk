...
class Job(SyncAPIResource):
    ...
    def create(self, project_id, name='', group_id=0):
        ...
        try:
            data = self._client.post(f'/openapi/v1/sandbox/job/create', json=data, params=self._client.params)
            data = data.json()
    ...
    def detail(self, job_id):
        ...
        response = self._client.get(f"/openapi/v1/sandbox/job/{job_id}")
    ...
    def insert(self, **kwargs):
        ...
        response = self._client.post("/openapi/v1/sandbox/job/add", json=camel_data)
        log.info(f'[insert] json={camel_data}, response={response.json()}')
        return response.json().get("data")

    def delete(self, job_id):
        # log.info(f"delete job {job_id}")
        response = self._client.post(f"/openapi/v1/sandbox/job/del/{job_id}")


    def terminate(self, job_id):
        # log.info(f"terminate job {job_id}")
        response = self._client.post(f"/openapi/v1/sandbox/job/terminate/{job_id}")


    def kill(self, job_id):
        # log.info(f"kill job {job_id}")
        response = self._client.post(f"/openapi/v1/sandbox/job/kill/{job_id}")
    

    def log(self, job_id, log_file="STDOUTERR", page=-1, page_size=8192):
        # log.info(f"log job {job_id}")
        response = self._client.get(
            f"/openapi/v1/sandbox/job/{job_id}/log",
    ...
    def create_job(
        ...
        response = self._client.post(f"/openapi/v1/sandbox/job/create", json=data)
